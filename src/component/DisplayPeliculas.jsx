...
const DisplayPeliculas = ({ search, input }) => {
  ...
  return (
    <>
      {input.search ? (
        data.length >= 1 ? (
          <Card.Text>{`Resultados de ${input.search}:`}</Card.Text>
        ) : (
          <Card.Text>{`Sin resultados para ${input.search}.`}</Card.Text>
        )
      ) : (
        <Card.Text>{`Lo mas trending del dia:`}</Card.Text>
      )}
      {data ? (
        data.map((search) => {
          return (
                ...
                <>
                  ...
                  search.backdrop_path ? (
                    <Card.Img
                      ...
                      alt={`imagen referente a la pelicula ${search.original_title}`}
                    />
                  ) : (
                  ...
                  {search.known_for ? (
                    <Stack gap={1} className="m-2 fs-6">
                      ...
                      {search.known_for.map((search) => {
                        return (
                          <Badge bg="secondary" key={`badge${search.id}`}>
        ...
        <Card.Title>Busca tu película</Card.Title>
      )}
    </>
  );
};
